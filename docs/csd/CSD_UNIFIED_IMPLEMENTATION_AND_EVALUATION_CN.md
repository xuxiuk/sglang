...
`scripts/csd/run_dflash_csd.sh`、`scripts/csd/run_dspark_csd.sh`、
`scripts/csd/merge_csd_tables.py`、`scripts/csd/eval_dspark_long.py` 和
`scripts/csd/summarize_dspark_task_metrics.py` 是新增的校准/评测工具，不参与 server 的在线执行；
...
本轮两组 GPU 的正式测试使用了与结果绑定的历史编排包。该编排包不再作为源码提交；
可复用的校准、服务启动和评测组件位于 `scripts/csd/`，原始编排及产物的来源见第 9 节。
...
GPU_SET=4,5,6,7 PORT=30000 \
  bash scripts/csd/run_dspark_csd.sh plain-server
```
...
`scripts/csd/calibrate_dspark_csd.py` 只读取其中的 `domain` 和 `prompt`，由当前服务重新生成
completion 并采集 pair。
...
DSpark 的通用 calibration 流程保存在 `scripts/csd/run_dspark_csd.sh`。DP=4 的 calibration
会先导出四个 rank shard，再由
`scripts/csd/merge_csd_tables.py` 按 pair 对频次求和。
...
| --- | --- | --- | --- | --- |
| Qwen3.5-35B-A3B / MTP | EAGLE 3-1-4 | 历史正式编排，见产物归档 | `qwen35b_mtp314` 归档中的 calibration table | 已用于当前 Qwen 正式评测；历史文件名和 CLI metadata 中的 `draft3` 表示 3 个 draft token，计入 verifier root 后对应报告中的 4-token 有效宽度，因此统一记作 3-1-4 |
| Qwen3.5-397B-A17B-FP8 / MTP | EAGLE 3-1-4 | 历史正式编排，见产物归档 | `qwen397b_mtp314` 归档中的 calibration table | 6000 prompts、512 max-new-tokens、并发8；正式加载阈值6 |
| DeepSeek-V4-Flash-DSpark / DSpark | compact DSpark，SPS table，DP=4 | `scripts/csd/run_dspark_csd.sh` | 由脚本输出到本地 `runs/` 或外部归档 | 四个 DP shard 合并后的正式 DSpark table |
| DeepSeek-V4-Flash / MTP | EAGLE 3-1-4，DeepSeek-V4-Flash-MTP-Draft，DP=4 | 历史正式编排，见产物归档 | `v4_mtp314` 归档中的 merged table | 6000 prompts，四 shard 频次守恒校验通过 |

Qwen MTP 和 DSpark 的 calibration 客户端共用：

```text
scripts/csd/calibrate_dspark_csd.py
```
...
```bash
bash scripts/csd/run_dspark_csd.sh formal-calibration-server
bash scripts/csd/run_dspark_csd.sh formal-calibration-run
bash scripts/csd/run_dspark_csd.sh export-table
```

DeepSeek-V4 MTP 3-1-4 的历史正式编排完成服务启动、6000 条请求、四 shard 导出、合并
和频次守恒校验。该一次性编排与大体积产物已移出源码历史；重新执行时应基于
`scripts/csd/` 的通用客户端和 table merge 工具建立新的、记录 source commit 的运行目录。
...
本分支不保留评测结果、模型输出、逐请求时序、服务日志或冻结评测环境。
`runs/` 目录默认被 Git 忽略；需要长期保存的结果应存放在仓库外，并同时记录 source
commit、完整命令行、数据集 revision 和 calibration-table digest。
...
历史主任务产物包含`artifacts/`、`logs/`和`results/classic_tree_shape_sweep.jsonl`；
...
```bash
bash scripts/csd/run_dspark_accuracy81920_matrix.sh
```

```text
runs/dspark_csd/accuracy81920_matrix_<timestamp>/
```
