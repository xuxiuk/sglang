#!/usr/bin/env bash
# Batch=1 accuracy/stability rerun for the 515 + calib1024 report methods.
# Runs all methods in one METHOD_SET on one TP=4 GPU group.
...
RUN_STAMP=${RUN_STAMP:-$(date +%Y%m%d_%H%M%S)_515_calib1024_batch1_all_methods}
OUT_ROOT_BASE=${OUT_ROOT_BASE:-"${REPO_ROOT}/benchmark/csd/runs/lighteval_csd_515_calib1024_batch1_all_methods/${RUN_STAMP}"}
MERGED_RESULT_FILE=${MERGED_RESULT_FILE:-"${OUT_ROOT_BASE}/results/batch1_all_methods.jsonl"}
...
METHOD_SET=${METHOD_SET:-"auto eagle plain plain_entropy_p20 dynamic dynamic_ignore_ratio dynamic_entropy_p20_ignore_ratio topkeep15000_no_ratio topkeep15000_p20_no_ratio"}
...
status=0
echo "CUDA_DEVICES=${CUDA_DEVICES} PORT=${PORT} TP_SIZE=${TP_SIZE} MAX_RUNNING_REQUESTS=${MAX_RUNNING_REQUESTS}"
echo "METHOD_SET=${METHOD_SET}"
echo "CSD_REBUILD_TOP_KEEP=${CSD_REBUILD_TOP_KEEP:-<empty>} (topkeep methods set this per method)"
...
env \
  ...
  >"${DRIVER_LOG_DIR}/batch1_all_methods.log" 2>&1 || status=$?
...
for row in rows:
    ...
    for method in (
        "topkeep15000_p20_no_ratio",
        "topkeep15000_no_ratio",
        ...
        methods.append(tag.split("_", 1)[0] if tag else "unknown")
