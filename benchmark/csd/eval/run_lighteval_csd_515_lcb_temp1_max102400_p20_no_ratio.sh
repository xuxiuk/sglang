#!/usr/bin/env bash
# LCB-only rerun for Qwen3.5-35B-A3B using the prior general reasoning
# sampling parameters and max_new_tokens=102400.
...
RUN_STAMP=${RUN_STAMP:-$(date +%Y%m%d_%H%M%S)_515_lcb_temp1_max102400_p20_no_ratio}
OUT_ROOT=${OUT_ROOT:-"${REPO_ROOT}/benchmark/csd/runs/lighteval_csd_515_lcb_temp1_max102400_p20_no_ratio/${RUN_STAMP}"}
RESULT_FILE=${RESULT_FILE:-"${OUT_ROOT}/results/lcb_temp1_max102400_p20_no_ratio.jsonl"}
...
env \
  ...
  METHOD_SET="${METHOD_SET:-auto eagle plain dynamic dynamic_ignore_ratio plain_entropy_p20 dynamic_entropy_p20_ignore_ratio}" \
  PLAIN_CSD_TABLE_PATH="${PLAIN_CSD_TABLE_PATH}" \
  CSD_DYNAMIC_UPDATE_IGNORE_PROB_RATIO="${CSD_DYNAMIC_UPDATE_IGNORE_PROB_RATIO:-1}" \
  LCB_RECOMMENDED_GEN_KWARGS="${LCB_RECOMMENDED_GEN_KWARGS:-temperature=1.0,top_p=0.95,top_k=20,min_p=0.0,presence_penalty=1.5,repetition_penalty=1.0}" \
  LCB_MAX_GEN_TOKS="${LCB_MAX_GEN_TOKS:-102400}" \
  LCB_MAX_LENGTH="${LCB_MAX_LENGTH:-120000}" \
