#!/usr/bin/env bash
# Supplement run for 515 + calib1024:
#   1. dynamic no-ratio + rebuild top_keep=15000
#   2. dynamic no-ratio + P20 entropy gate + rebuild top_keep=15000
#
# This intentionally does not rerun auto/eagle/plain. The two supplement
# methods run sequentially on one TP=4 GPU group.
...
RUN_STAMP=${RUN_STAMP:-$(date +%Y%m%d_%H%M%S)_515_calib1024_topkeep15000_p20_no_ratio_supplement}
OUT_ROOT_BASE=${OUT_ROOT_BASE:-"${REPO_ROOT}/benchmark/csd/runs/lighteval_csd_515_calib1024_topkeep15000_p20_no_ratio_supplement/${RUN_STAMP}"}
MERGED_RESULT_FILE=${MERGED_RESULT_FILE:-"${OUT_ROOT_BASE}/results/topkeep15000_p20_no_ratio_supplement.jsonl"}
...
METHOD_SET=${METHOD_SET:-"dynamic_ignore_ratio dynamic_entropy_p20_ignore_ratio"}
...
echo "CSD_REBUILD_TOP_KEEP=${CSD_REBUILD_TOP_KEEP} CSD_DYNAMIC_UPDATE_IGNORE_PROB_RATIO=${CSD_DYNAMIC_UPDATE_IGNORE_PROB_RATIO}"
...
env \
  ...
  >"${DRIVER_LOG_DIR}/supplement.log" 2>&1 || status=$?
