...
CSD_TARGET_RETAIN_PERCENT=${CSD_TARGET_RETAIN_PERCENT:-none}
...
CSD_REBUILD_TOP_KEEP=${CSD_REBUILD_TOP_KEEP:-15000}
...
METHOD_SET=${METHOD_SET:-"baseline vanilla csd_plain_table_static csd_plain_table csd_plain_table_top5"}
...
config = {
    ...
    "csd_dynamic_update_ignore_prob_ratio": os.environ.get("CSD_DYNAMIC_UPDATE_IGNORE_PROB_RATIO") == "1",
...
run_eval() {
  ...
  local lighteval_task task_name run_name output_path metrics_path tracker_dir config_json log_path
  lighteval_task=$(lighteval_task_spec "${task_spec}")
  task_name=$(safe_name "${lighteval_task}")
  local ignore_ratio_suffix="respectratio"
  if [[ "${CSD_DYNAMIC_UPDATE_IGNORE_PROB_RATIO}" == "1" ]]; then
    ignore_ratio_suffix="ignoreratio"
  fi
  run_name=$(safe_name "${method}_${task_slug}_${gen_variant}_steps${SPEC_NUM_STEPS}_topk${SPEC_TOPK}_draft${SPEC_DRAFT_TOKENS}_key${CSD_KEY_SELECTION_STRATEGY}_target${CSD_TARGET_RETAIN_PERCENT}pct_freq${CSD_FREQ_THRESHOLD}_score${CSD_SCORE_THRESHOLD}_ratio${CSD_PROB_RATIO}_tableratio${CSD_TABLE_PROB_RATIO}_topkeep${top_keep:-none}_entropy${CSD_FORCE_ACCEPT_ENTROPY_THRESHOLD:-none}_${ignore_ratio_suffix}_${RUN_STAMP}")
  ...
  if [[ "${mode}" == "csd" ]]; then
    extra_args+=(
      ...
      --csd-key-selection-strategy "${CSD_KEY_SELECTION_STRATEGY}"
      --csd-score-threshold "${CSD_SCORE_THRESHOLD}"
    ...
    if [[ -n "${CSD_FORCE_ACCEPT_ENTROPY_THRESHOLD}" ]]; then
      extra_args+=(--csd-force-accept-entropy-threshold "${CSD_FORCE_ACCEPT_ENTROPY_THRESHOLD}")
    fi
    if [[ -n "${top_keep}" ]]; then
  ...
  echo "Running method=${method}, task=${lighteval_task}, gen_variant=${gen_variant}, enable_thinking=${enable_thinking}, output=${output_path}, log=${log_path}"
  (
...
run_method_set() {
  ...
  local max_length="$6"
  local methods=()
  read -r -a methods <<<"${METHOD_SET}"
  local method status

  if ! task_enabled "${task_slug}"; then
    return 0
  fi
