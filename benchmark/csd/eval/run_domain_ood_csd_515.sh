#!/usr/bin/env bash
# Run prepared Alpaca-style domain OOD datasets with the 515 CSD setup.
...
RESULT_FILE=${RESULT_FILE:-"${OUT_ROOT}/results/domain_ood_csd_515.jsonl"}
...
server_args_for_method() {
  ...
  case "${method}" in
    ...
    plain)
      ...
      ;;
    dynamic_ignore_ratio|dynamic_entropy_p20_ignore_ratio|dynamic_ignore_ratio_huge|dynamic_entropy_p20_ignore_ratio_huge)
...
"${PYTHON}" - <<'PY' "${OOD_DATA_FILES}" | tee "${OUT_ROOT}/dataset_counts.txt" | tee -a "${OUT_ROOT}/run_config.txt"
import json
import sys
...
"${PYTHON}" benchmark/csd/eval/summarize_domain_ood_csd.py "${RESULT_FILE}" | tee "${OUT_ROOT}/summary.md"
