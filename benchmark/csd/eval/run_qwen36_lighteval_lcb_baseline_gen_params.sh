...
MODEL_PATH=${MODEL_PATH:-/home/shared/models/Qwen/Qwen3.6-35B-A3B}
...
run_baseline_variant() {
  ...
  local config_json
  config_json=$(experiment_config_json "${variant}" "${tasks}" "${gen_kwargs}" "${enable_thinking}" "${max_gen_toks}" "${max_length}")
  ...
  echo "Running baseline variant=${variant}, tasks=${tasks}, enable_thinking=${enable_thinking}, max_gen_toks=${max_gen_toks}, gen_kwargs=${gen_kwargs}"
...
run_task_variants() {
  ...
  run_baseline_variant "${task_slug}_thinking_recommended" "${task_spec}" "${recommended_kwargs}" true "${max_gen_toks}" "${max_length}"
  run_baseline_variant "${task_slug}_thinking_temp0" "${task_spec}" "${temp0_kwargs}" true "${max_gen_toks}" "${max_length}"
  run_baseline_variant "${task_slug}_thinking_temp1" "${task_spec}" "${temp1_kwargs}" true "${max_gen_toks}" "${max_length}"
  run_baseline_variant "${task_slug}_nonthinking" "${task_spec}" "${non_think_kwargs}" false "${max_gen_toks}" "${max_length}"
}
