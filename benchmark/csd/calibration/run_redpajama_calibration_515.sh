#!/usr/bin/env bash
# Reproduce the RedPajama calibration table used by the v0.5.10 5/1/5 run.
...
RUN_STAMP=${RUN_STAMP:-$(date +%Y%m%d_%H%M%S)_redpajama_calibration_515}
OUT_DIR=${OUT_DIR:-${REPO_ROOT}/benchmark/csd/runs/${RUN_STAMP}}
...
SGLANG_TORCH_PROFILER_DIR=${SGLANG_TORCH_PROFILER_DIR:-${OUT_DIR}/profiles}
...
CSD_TABLE_PATH=${CSD_TABLE_PATH:-"${REPO_ROOT}/benchmark/csd/assets/calibration/csd_table_redpajama_${CSD_TABLE_KIND}_${DOMAIN_COUNT}domains_n${SAMPLES_PER_DOMAIN}_${MODEL_NAME_PART}_${DRAFT_MODEL_NAME_PART}_${SPEC_ALGORITHM_PART}_${SPEC_SHAPE_PART}_temp${TEMPERATURE}_ratio${CSD_PROB_RATIO}_maxnew${MAX_NEW_TOKENS}.json"}
...
"${PYTHON}" benchmark/csd/calibration/bench_redpajama_csd.py \
