...
from sglang.jit_kernel.dsv4 import CompressorPrefillPlan, compress_forward
...
@pytest.mark.parametrize("verify_width", [2, 4, 5, 6, 8, 12])
def test_paged_prefill_dspark_rewrites_full_verify_window(
    planner_on_device: bool,
    verify_width: int,
) -> None:
    """Every DSpark verifier row must be present in the state write plan."""
