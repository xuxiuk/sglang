...
OUT_ROOT_OVERRIDE=${OUT_ROOT_OVERRIDE:-${RUN_DIR}/results}
RUN_STAMP_OVERRIDE=${RUN_STAMP_OVERRIDE:-}
...
run_dataset() {
  ...
  local nccl_port=$4
  local data_file=$5
  local task_max_new_tokens=$MAX_NEW_TOKENS
  if [[ "$task" == "taco" && "$MAX_NEW_TOKENS" == "81920" ]]; then
    # The retained final TACO run used the 40960-token cap.
    task_max_new_tokens=40960
  fi
  ...
  env \
    ...
    RUN_STAMP="${RUN_STAMP_OVERRIDE:-${task}_n4_314_c48_20260802}" \
