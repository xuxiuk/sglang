...
EVAL_ROOT=${EVAL_ROOT:-${SCRIPT_DIR}/..}
cd "${REPO_ROOT}"
...
for data_file in ${OOD_DATA_FILES}; do
  ...
  for method in ${METHOD_SET}; do
    ...
    metrics_pid=""
    if [[ "${ENABLE_SERVER_METRICS}" == "true" ]]; then
      "${PYTHON}" "${EVAL_ROOT}/scrape_sglang_decode_metrics.py" \
        --host "${HOST}" \
        --port "${PORT}" \
    ...
    bench_args=(
      "${EVAL_ROOT}/bench_sglang_chat_generation.py"
  ...
  "${PYTHON}" "${EVAL_ROOT}/summarize_domain_ood_csd.py" "${result_file}" \
...
"${PYTHON}" "${EVAL_ROOT}/summarize_domain_ood_csd.py" "${combined_result_file}" \
