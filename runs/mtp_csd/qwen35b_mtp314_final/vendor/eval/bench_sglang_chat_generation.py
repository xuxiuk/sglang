...
- apps / taco: load prepared local code-OOD generation files
...
def prompt_from_row(dataset: str, row: dict[str, Any]) -> str:
    if dataset in {"alpaca_eval", "apps", "taco"}:
            ...
            payload.append(
                {
                    ...
                    "question_index": row.get("__question_index"),
                    "sample_index": row.get("__sample_index"),
                    "instruction": row["instruction"],
                    "output": answer,
                    "generator": model_id,
                }
...
def write_request_metrics(
    ...
    with output_path.open("w", encoding="utf-8") as fout:
        for idx, row in enumerate(rows):
            ...
            record = {
                "index": idx,
                "question_index": row.get("__question_index", idx),
                "sample_index": row.get("__sample_index", 0),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Independent generations per logical question.",
    )
    ...
    logical_rows = all_rows[args.offset :]
    if args.num_examples is not None:
        logical_rows = logical_rows[: args.num_examples]
    if args.num_samples < 1:
        raise ValueError("--num-samples must be at least 1")
    rows = []
    for question_index, row in enumerate(logical_rows):
        for sample_index in range(args.num_samples):
            expanded = dict(row)
            expanded["__question_index"] = question_index
            expanded["__sample_index"] = sample_index
    ...
    result = {
        ...
        "num_questions": len(logical_rows),
