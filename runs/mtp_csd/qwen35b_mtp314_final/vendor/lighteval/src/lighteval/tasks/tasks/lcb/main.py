...
from lighteval.metrics.metrics import Metrics, SampleLevelMetricGrouping
...
def prepare_prompt(line: dict[str, Any]) -> str:
    ...
    if starter_code := line.get("starter_code", None):
        query += "### Format: You will use the following starter code to write the solution to the problem and enclose your code within delimiters.\n"
        query += f"```python\n{starter_code}\n```\n\n"
    else:
        query += "### Format: Read the inputs from stdin solve the problem and write the answer to stdout (do not directly test on the sample inputs). Enclose your code within delimiters as follows. Ensure that when the python program runs, it reads the inputs, runs the algorithm and writes output to STDOUT.\n"
        query += "```python\n# YOUR CODE HERE\n```\n\n"
    query += "### Answer: (use the provided format with backticks)\n\n"
    return query
...
class CodegenMetric(SamplingMetric, SampleLevelComputation):
    ...
    def compute(self, model_response: ModelResponse, doc: Doc, **kwargs) -> dict:
        ...
        return {
            "codegen_pass@1:avg4": metrics["pass@1"],
            "codegen_pass@4": metrics["pass@4"],
        }


lcb_codegen_metric = SampleLevelMetricGrouping(
    metric_name=["codegen_pass@1:avg4", "codegen_pass@4"],
    category=SamplingMethod.GENERATIVE,
    higher_is_better={
        "codegen_pass@1:avg4": True,
        "codegen_pass@4": True,
    },
    sample_level_fn=CodegenMetric(),
    corpus_level_fn={
        "codegen_pass@1:avg4": np.mean,
        "codegen_pass@4": np.mean,
    },
