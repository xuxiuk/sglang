...
class MockStdinWithBuffer:
    """stdin replacement compatible with solutions using ``sys.stdin.buffer``.

    This mirrors LiveCodeBench's current evaluator.  Competitive-programming
    solutions commonly read bytes through ``sys.stdin.buffer``; replacing
    stdin with a plain ``StringIO`` incorrectly turns those valid solutions
    into runtime errors.
    """
...
def check_correctness(sample, generation, timeout: int) -> list[int | bool]:
    ...
    ctx = _eval_mp_context()
    manager = ctx.Manager()
    result = manager.list()
    p = ctx.Process(target=_temp_run, args=(sample, generation, result))
    p.start()
...
def evaluate_generations(
    ...
    with tqdm(total=len(inputs)) as pbar:
        with ProcessPoolExecutor(max_workers=num_process_evaluate, mp_context=_eval_mp_context()) as executor:
