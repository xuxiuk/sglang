...
class CSDTableBuilder : public torch::CustomClassHolder {
 ...
 private:
  static torch::Tensor checked_input(const torch::Tensor& input, const char* name) {
    TORCH_CHECK(input.device().is_cpu(), "CSD ", name, " must be on CPU");
    TORCH_CHECK(input.scalar_type() == torch::kInt64, "CSD ", name, " must be int64");
...
std::tuple<torch::Tensor, int64_t> csd_build_hash_table_cpu(
  ...
  TORCH_CHECK(input_keys.scalar_type() == torch::kInt64, "CSD keys must be int64");
  TORCH_CHECK(input_keys.dim() == 1, "CSD keys must be one-dimensional");
  TORCH_CHECK(max_probe >= 1, "CSD max_probe must be at least 1");
  TORCH_CHECK(load_factor > 0.0 && load_factor <= 1.0, "CSD load_factor must be in (0, 1]");
  ...
  int64_t num_entries = 0;
  while (true) {
    auto [built, entries] = try_build(key_data, num_keys, capacity, max_probe, table);
    if (built) {
      num_entries = entries;
      break;
    }
