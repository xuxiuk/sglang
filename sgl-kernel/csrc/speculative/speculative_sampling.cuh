...
__device__ __forceinline__ IdType CsdSampleResidualTokenAndMaxLogit(
    ...
    DType* draft_probs,
    DType* target_logits,
    uint32_t cur_target_prob_offset,
    uint32_t cur_draft_prob_offset,
    uint32_t d,
    DType coin,
  ...
  for (uint32_t i = 0; i < ceil_div(d, BLOCK_THREADS * VEC_SIZE); ++i) {
    ...
    if ((i * BLOCK_THREADS + tx) * VEC_SIZE < d) {
      q_vec.load(target_probs + cur_target_prob_offset + i * BLOCK_THREADS * VEC_SIZE + tx * VEC_SIZE);
      if (load_draft_probs) {
        p_vec.load(draft_probs + cur_draft_prob_offset + i * BLOCK_THREADS * VEC_SIZE + tx * VEC_SIZE);
      }
    }
    if constexpr (COMPUTE_MAX_LOGIT) {
      ...
      if ((i * BLOCK_THREADS + tx) * VEC_SIZE < d) {
        logits_vec.load(target_logits + cur_target_prob_offset + i * BLOCK_THREADS * VEC_SIZE + tx * VEC_SIZE);
      }
#pragma unroll
...
__global__ void TreeSpeculativeSamplingTargetOnly(
    ...
    DType csd_force_accept_entropy_min_threshold,
  ...
  uint32_t cur_draft_prob_offset = bx * num_draft_prob_rows * d;
  ...
  for (uint32_t j = 1; j < num_speculative_tokens; ++j) {
    ...
    while (cur_index != -1) {
      ...
      DType draft_prob_single = draft_probs[cur_draft_prob_offset + draft_token_id];
      prob_acc += target_prob_single;

      bool normal_accept = use_rejection_sampling
                               ? coin * draft_prob_single < target_prob_single
      ...
      if (!normal_accept) {
        ...
        if (!use_rejection_sampling) {
          draft_probs[cur_draft_prob_offset + draft_token_id] = target_prob_single;
        }
        ...
        if constexpr (CSD_ENABLED || CSD_DYNAMIC_UPDATE) {
          ...
          if (tx == 0) {
            ...
            if (csd_force_accept &&
                (csd_force_accept_entropy_threshold >= DType(0) ||
                 csd_force_accept_entropy_min_threshold >= DType(0))) {
          ...
          if (should_check_csd_entropy) {
            ...
            if (tx == 0) {
              bool entropy_min_pass = csd_force_accept_entropy_min_threshold < DType(0) ||
                                      target_entropy >= csd_force_accept_entropy_min_threshold;
              bool entropy_max_pass = csd_force_accept_entropy_threshold < DType(0) ||
                                      target_entropy <= csd_force_accept_entropy_threshold;
              temp_storage.block_aggregate.value = entropy_min_pass && entropy_max_pass ? DType(1) : DType(0);
            }
            __syncthreads();
          }
      ...
      if (normal_accept || csd_force_accept) {
        ...
        cur_draft_prob_offset = (bx * num_draft_prob_rows + cur_index) * d;
...
cudaError_t LaunchTreeSpeculativeSamplingTargetOnly(
  ...
  void* args[] = {
      ...
      &csd_force_accept_entropy_min_threshold,
...
cudaError_t TreeSpeculativeSamplingTargetOnly(
    ...
    DType csd_force_accept_entropy_min_threshold = -1.0f,
    bool use_rejection_sampling = false,
